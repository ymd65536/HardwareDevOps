...
SCAD_PATH = REPO_ROOT / "src" / "copilot-stand.scad"


def parse_ascii_stl_bounding_box(stl_path: Path):
    min_x = min_y = min_z = float("inf")
    max_x = max_y = max_z = float("-inf")
    with stl_path.open("r", encoding="utf-8", errors="replace") as handle:
        for line in handle:
            stripped = line.strip()
    ...
    if not (float("-inf") < max_x < float("inf")):
        raise AssertionError(f"No vertex data found in STL: {stl_path}")
...
class ManufacturingReportTests(unittest.TestCase):
    ...
    def test_stl_bounding_box_matches_expected_dimensions(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir)
            stl_path = tmpdir_path / "generated.stl"
            ...
            bounds = parse_ascii_stl_bounding_box(stl_path)

            self.assertAlmostEqual(bounds["x_max"] - bounds["x_min"], 90.0, delta=0.5)
            self.assertAlmostEqual(bounds["y_max"] - bounds["y_min"], 70.0, delta=0.5)
            self.assertAlmostEqual(bounds["z_max"] - bounds["z_min"], 46.0, delta=0.5)
